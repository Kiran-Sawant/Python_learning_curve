...
async def get_fabric_links(html_body:str)-> list:
    """Grabs links from a list view page, that leads to 
    fabrics detail view page.
    Returns a list of dicionaries, each containing, fabric id,
    slug, relative link, scraped status."""
...
async def scraper(url:str, i=-1, timeout:int=60)-> list:
...
async def run(urls:list, timeout:int=60)->list:
    """
    attr:
        urls(list): list of URLs of webpages to scrape.
        timeout(int): timeout for async."""

    # creating a list to add coroutines
    site_links = list()

    # populating the list with coroutines
    for i, url in enumerate(urls):
        site_links.append(
            asyncio.create_task(scraper(url, i=i, timeout=timeout))
        )
    
    # .gather() takes a sequence of coroutines and runs them concurrently.
    # it will return a list of return values of all the coroutines.
    # we are passing it an unpacked list of coroutines.
...
if __name__ == "__main__":
    ...
    results = asyncio.run(run(urls, timeout=30))
