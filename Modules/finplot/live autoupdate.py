...
# URL for bitmex socket.
baseurl = 'https://www.bitmex.com/api'

# list of plots on the chart
plots = []

#_________Getting historical price data_______________#
def price_history(load_periods=5, interval_mins=60):
    ...
    d = requests.get(url).json()        # converting the received JSON to dict()
    # drop volume and status
    del d['v']
    del d['s']
    return d

#_____calculate bollinger band & add o df____#
# def calc_bollinger_bands(df):
#     r = df['c'].rolling(20)
#     df['bbh'] = r.mean() + 2*r.std()
#     df['bbl'] = r.mean() - 2*r.std()

#_______Update plot with new data_____#
# def update_plot():
...
#_______appends new data rows to the historical price DataFrame_______#
#_______Runns on a Separate therad_________#
...
#     update_plot()
#     fplt.timer_callback(update_plot, 5.0) # update every second
#     fplt.show()
