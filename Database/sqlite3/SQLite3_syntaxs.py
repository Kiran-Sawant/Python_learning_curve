"""Working wit SQLite DB in python requires a connection object similar to file I/O objects,
it can be created using sqlite3 module.

PyDoc: https://docs.python.org/3.7/library/sqlite3.html#controlling-transactions"""
...
# print(type(k))            # k is a sqlite3 Cursor() object
...
""" .executemany() is used when there are multiple
values to be inserted into multiple rows. One needs
an iterable with the values that are needed to be stored
and put a ? as a place-holder in the SQL statement
where those values are needed to be put.
